...
function main {
   ...
   if [[ $? == 0 ]]; then
      git log --since=3.days.ago --pretty=format:'{%n "version": "'$version'",%n "code": "'$code'",%n "date": "%ad",%n "message": "%f"%n},' $version...$prev
      main $prev^
   else # show all to first commit
      entries=$(git log --since=3.days.ago --pretty=format:'{%n "version": "'$version'",%n "code": "'$code'",%n "date": "%ad",%n "message": "%f"%n},' $version...$(git rev-list --max-parents=0 HEAD))
